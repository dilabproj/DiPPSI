...
from configs.test_csdi_configs import *
...
if __name__=='__main__':
    ...
    #configs = [lininterp_scientisst_ppg_testtransient_30percent]
    configs = [csdi_scientisst_ecg_testtransient_30percent]
    ...
    for config in configs:
        ...
        X_test, Y_dict_test = load(**config["data_load"])
        ...
        if os.path.exists(os.path.join(path, "imputation.npy")):
            ...
            # # 使用torch.isnan()检查每个元素是否为NaN
            # isnan_mask = np.isnan(imputation)

            # # print("Original Data: ", )
            # print("Is NaN: ", isnan_mask)
        ...
        if bootstrap is not None: # only support for mimic right now
            ...
            if torch.isnan(mse_losses).any():
                print("mse_losses contains NaN values.")
            ...
            if "mimic" in config["data_name"]:
                ...
                sens_bootstraplist = []
                prec_bootstraplist = []
            elif "scientisst" in config["data_name"]:
                stats = eval_heartbeat_detection(imputation=imputation, target_seq=Y_dict_test["target_seq"], 
                                                 input=X_test, path=path, return_stats=True)
                sens = np.nanmean(stats[:,0])
                prec = np.nanmean(stats[:,1])
                ...
                # Ensure both imputation and target_seq are either numpy arrays or tensors
                if isinstance(Y_dict_test["target_seq"], torch.Tensor):
                    target_seq_array = Y_dict_test["target_seq"].numpy()  # Convert tensor to numpy
                    mae_losses = np.abs(imputation - target_seq_array)
                else:
                    # Log a message if the types are unexpected
                    printlog("Unexpected data types for imputation or target_seq", path)
                
                # Calculate MAPE
                non_zero_actuals = np.where(target_seq_array != 0, target_seq_array, 1e-8)  # Avoid division by zero
                mape_losses = 100 * (mae_losses / non_zero_actuals)
                mape = np.nanmean(mape_losses)  # Use nanmean to safely handle NaNs
                printlog(f"MAPE: {mape}", path)

                if not np.isnan(f1) and not np.isnan(prec) and sens != 0:
                    adjusted_f1 = f1 * 100  # Adjust F1 score to percentage
                    cprs = (adjusted_f1 + (100 - mape)) / 2
                    printlog(f"CPRS: {cprs}", path)
                else:
                    printlog("Cannot calculate CPRS due to NaN or zero values", path)
            ...
            if "mimic" in config["data_name"]:
                ...
                # Option 1: Save each list to its own file
                np.save(os.path.join(path, 'mse_losses_bootstraplist.npy'), mse_losses_bootstraplist)
                np.save(os.path.join(path, 'f1_bootstraplist.npy'), f1_bootstraplist)
                np.save(os.path.join(path, 'prec_bootstraplist.npy'), prec_bootstraplist)
                np.save(os.path.join(path, 'sens_bootstraplist.npy'), sens_bootstraplist)
            else:
