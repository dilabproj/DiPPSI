    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, activation="relu", feedforward="fc",
        ...
        if trans_bottleneck_size:
            ...
            self.self_attn = MultiheadAttention_CustomAttn(trans_bottleneck_size, nhead, output_dim=d_model, dropout=dropout, custom_attn=custom_attn, 
                                                        ...
                                                        bias=bias, time_decay=time_decay,max_len=max_len)
        else:
...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model, dropout=0.1, max_len=50000,kernel_size=21):
    ...
    def forward(self, x):
        x = x + self.pe.unsqueeze(0) #[:x.size(1), :].unsqueeze(0)
...
class MultiheadAttention_CustomAttn(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0., custom_attn="cosine_similarity", 
        ...
        if incepattn_bot:
            ...
            if q_k_func:
                ...
                if output_dim:
                    self.q_k_conv = nn.Conv1d(embed_dim, output_dim*2, kernel_size=1, padding=0, bias=bias)
                else:
                    self.q_k_conv = nn.Conv1d(embed_dim, embed_dim*2, kernel_size=1, padding=0, bias=bias)
        
        ##for deep MVI
        #linear layer for value_left,value_right
        self.mapping_query = nn.Linear(2*embed_dim,embed_dim)
        self.pos_encoder = PositionalEncoding(embed_dim, 0.1,max_len)     
    ...
    def forward(self, query, pos_embed, return_attn_weights=False):
        ...
        # self-attention
        #value_left, value_right here 

        value_left = torch.zeros(query.shape).to(query.device)
        value_right = torch.zeros(query.shape).to(query.device)
        value_left[1:,:,:] = query[:-1,:,:]
        value_right[:-1,:,:] = query[1:,:,:]
        attn_query = self.pos_encoder(self.mapping_query(torch.cat([value_left,value_right],dim=2).transpose(1,0))).transpose(0,1)
        
        if self.inception:
            q = self.q_conv(attn_query.permute(1,2,0))
            k = self.k_conv(attn_query.permute(1,2,0))
        else: 
            q, k = self.q_k_conv(attn_query.permute(1,2,0)).chunk(2, dim=1) # query should be [batch size, channel, length]
