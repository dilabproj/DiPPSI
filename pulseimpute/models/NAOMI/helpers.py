...
def run_epoch(train, model, exp_data, clip, optimizer=None, batch_size=64, num_missing=None, teacher_forcing=True):
    ...
    while i + batch_size <= exp_data.shape[0]:
        ...
        ind = torch.from_numpy(inds[i:i+batch_size]).int()
        i += batch_size
        data = exp_data[ind]
        ...
        missing_list = torch.from_numpy(np.random.choice(np.arange(1, data.shape[0]), num_missing, replace=False)).int()
...
def update_discrim(discrim_net, optimizer_discrim, discrim_criterion, exp_states, exp_actions, \
    ...
    if dis_times > 0:
        return g_o_ave / dis_times, e_o_ave / dis_times, total_discrim_loss
...
def collect_samples_interpolate(policy_net, expert_data, use_gpu, i_iter, task, size=64, name="sampling_inter", draw=False, stats=False, num_missing=None):
    ...
    exp_ind = torch.from_numpy(np.random.choice(expert_data.shape[0], size)).int()
    data = expert_data[exp_ind].clone()
    seq_len = data.shape[1]
    #print(data.shape, seq_len)
    if use_gpu:
        data = data.cuda()
    data = Variable(data.transpose(0, 1)) # seq_len, batch_size, dim
    # data = Variable(data.squeeze().transpose(0, 1))
    ground_truth = data.clone()
    ...
    missing_list = torch.from_numpy(np.random.choice(np.arange(1, seq_len), num_missing, replace=False)).int()
