...
class naomi():
    def __init__(self,modelname, data_name, train_data=None, val_data=None, 
        ...
        if train_realecg or train_realppg:
            if train_realppg:
                tuples_path_train = os.path.join("/mnt/1stHDD/nfs/roeywu/PPG_data", "missingness_patterns/mHealth_missing_ppg", f"missing_ppg_train.csv")
                tuples_path_val = os.path.join("/mnt/1stHDD/nfs/roeywu/PPG_data", "missingness_patterns/mHealth_missing_ppg", f"missing_ppg_val.csv")
            elif train_realecg:
                tuples_path_train = os.path.join("/mnt/1stHDD/nfs/roeywu/PPG_data", "missingness_patterns/mHealth_missing_ecg", f"missing_ecg_train.csv")
                tuples_path_val = os.path.join("/mnt/1stHDD/nfs/roeywu/PPG_data", "missingness_patterns/mHealth_missing_ecg", f"missing_ecg_val.csv")
            ...
            while disc_iter < self.pretrain_disc_iter:
                for local_batch, local_label_dict in self.train_loader:
                    ...
                    if torch.isnan(local_batch).any():
                        print(f"Error: NaN encountered in local_batch at iteration {disc_iter}")
                        continue  # Skip this batch if NaNs are present

                    if torch.isnan(local_label_dict["original"]).any():
                        print(f"Error: NaN encountered in local_label_dict['original'] at iteration {disc_iter}")
                        continue  # Skip this batch if NaNs are present

                    local_batch, mask = create_missingness(local_batch, self.train_impute_wind, self.train_impute_prob, self.train_impute_extended, self.list_of_miss_train)
                    _, _, _, _, _, model_seq, _, _ = \
                        self.naomi_policy(local_batch, local_label_dict["original"], sample=True)
                    if torch.isnan(model_seq).any():
                        print(f"Error: NaN encountered in model_seq at iteration {disc_iter}")
                        continue  # Skip this batch if NaNs are present
                            ...
                            for local_batch, local_label_dict in tqdm(self.val_loader, desc="Validating", leave=False):
                                if self.val_iters is not None:
                                    if val_iter >= self.val_iters:
                                        break
                                isnan_in_data = torch.isnan(local_batch).any() or torch.isnan(local_label_dict["original"]).any()
                                if isnan_in_data:
                                ...
                                # exp_states is the first 999 steps and exp actions is the last 999 steps of the original seq 
                                mask = np.transpose(mask, (1,0,2)) == 0 # mask now for missingness 
                                # def manual_mse_loss(output, target, mask):
                                #     diff = output[mask] - target[mask].cuda()
                                #     square_diff = diff.pow(2)
                                #     if torch.isnan(square_diff).any():
                                #         print("NaN detected in square difference")
                                #     mse = square_diff.mean()
                                #     if torch.isnan(mse).any():
                                #         print("NaN detected in MSE")
                                #     return mse
                                def safe_mse_loss(output, target, mask):
                                    if torch.isnan(output).any() or torch.isnan(target).any():
                                        print("NaN detected in MSE inputs")
                                        return torch.tensor(0.0)  # Replace with a dummy value
                                    else:
                                        return nn.MSELoss()(output[mask], target[mask].cuda())
                                assert not torch.isnan(local_label_dict["original"]).any(), "Original labels contain NaNs"
                                assert not torch.isnan(model_seq).any(), "Model predictions contain NaNs"
                                assert not torch.isinf(local_label_dict["original"]).any(), "Original labels contain Infs"
                                assert not torch.isinf(model_seq).any(), "Model predictions contain Infs"
                                output_masked = model_seq[mask]
                                target_masked = local_label_dict["original"].transpose(0,1)[mask].cuda()
                                # print("Mask sum:", mask.sum().item())
                                if output_masked.nelement() == 0:
                                    # print("Warning: No elements to compute MSE on, skipping batch.")
                                    continue  # Skip this batch
                                if mask is not None:
                                    if torch.isnan(model_seq[mask]).any() or torch.isnan(local_label_dict["original"].transpose(0,1)[mask]).any():
                                        print("NaN detected after applying mask")
                                mseloss = safe_mse_loss(model_seq, local_label_dict["original"].transpose(0,1), mask)
                                total_val_mseloss += mseloss.item()
                                val_iter += 1
                            if self.val_iters is not None:
                                print("self.val_iters = ", self.val_iters)
                                total_val_mseloss /= self.val_iters
                            else:
                                print("len(self.val_loader) = ", self.val_loader)
    ...
    def data_loader_setup(self, train_data, val_data=None, imputation_dict=None):
        ...
        if val_data is not None:
            ...
            self.train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=self.bs, shuffle=True, num_workers=0, drop_last=True)

            val_dataset = pulseimpute_dataset(waveforms=val_data)
            self.val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=self.bs, shuffle=False, num_workers=0, drop_last=True)
        else:
            test_dataset = pulseimpute_dataset(waveforms=train_data, imputation_dict=imputation_dict)
            self.test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=self.bs, shuffle=False, num_workers=0, drop_last=False)
...
def create_missingness(waveforms, train_impute_wind, train_impute_prob, train_impute_extended, list_of_miss):
    ...
    if list_of_miss:
        ...
        mask = miss_matrix
        # Modified line to clip indices:
        valid_indices = np.where(miss_matrix == 0)
        clipped_indices = (valid_indices[0], np.clip(valid_indices[1], 0, waveforms.shape[1]-1))
        waveforms[clipped_indices] = 0
    ...
    # Trim the larger tensor
    if mask.shape[1] > waveforms.shape[1]:
        mask = mask[:, :waveforms.shape[1]]  # Trim mask to match waveforms size
    else:
        waveforms = waveforms[:, :mask.size(1)]  # Trim waveforms to match mask size
