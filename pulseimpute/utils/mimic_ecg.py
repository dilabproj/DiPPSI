...
def load(train=True, val=True, test=False, addmissing=False, path=os.path.join("/mnt/2ndHDD/nfs/roeywu/PPG_data/data/pulseimpute_data/waveforms/mimic_ecg")):
...
def modify(X, addmissing=False, type=None):
    ...
    if addmissing: # this is only activated during testing
        miss_tuples_path = os.path.join("/mnt/1stHDD/nfs/roeywu/PPG_data","missingness_patterns/mHealth_missing_ecg", f"missing_ecg_{type}.csv")
