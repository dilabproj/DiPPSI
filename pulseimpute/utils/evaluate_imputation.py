...
def eval_mse(imputation, target_seq, path, return_stats=False):
    ...
    if torch.isnan(mse_loss).any():
        print("mse_loss contains NaN values.")
...
def eval_heartbeat_detection(imputation, target_seq, input, path, return_stats=False):
    ...
    def get_groundtruths():
        ...
        # with open(f'/mnt/1stHDD/nfs/roeywu/PPG_data/data/mimic_{type}/peaks_list.pickle', 'wb') as fp:
        #     pickle.dump(r_peaks_list, fp)
        with open(f'/mnt/1stHDD/nfs/roeywu/PPG_data/data/scientisst_{type}/peaks_list.pickle', 'wb') as fp:
            pickle.dump(r_peaks_list, fp)

        return r_peaks_list
    # if not os.path.exists(f'/mnt/1stHDD/nfs/roeywu/PPG_data/data/mimic_{type}/peaks_list.pickle'):
    #     r_peaks_list = get_groundtruths()
    # else:
    #     with open(f'/mnt/1stHDD/nfs/roeywu/PPG_data/data/mimic_{type}/peaks_list.pickle', 'rb') as fp:
    #         r_peaks_list = pickle.load(fp)
    if not os.path.exists(f'/mnt/1stHDD/nfs/roeywu/PPG_data/data/scientisst_{type}/peaks_list.pickle'):
        r_peaks_list = get_groundtruths()
    else:
        with open(f'/mnt/1stHDD/nfs/roeywu/PPG_data/data/scientisst_{type}/peaks_list.pickle', 'rb') as fp:
            r_peaks_list = pickle.load(fp)
...
def eval_cardiac_classification(imputation, path):
    ...
    datafolder = '/mnt/1stHDD/nfs/roeywu/PPG_data/data/ptbxl_ecg/'
