...
if __name__=='__main__':
    ...
    configs = [fft_wireless_ppg_testtransient_30percent]
    #configs = [lininterp_wireless_ppg_test,fft_wireless_ppg_test]
    ...
    for config in configs:
        ...
        if bootstrap is not None: # only support for mimic right now
            ...
            if "mimic" in config["data_name"]:
                ...
                # Calculate MAPE
                non_zero_actuals = np.where(target_seq_array != 0, target_seq_array, 1e8)  # Avoid division by zero
                mape_losses = 100 * (mae_losses / np.abs(non_zero_actuals))
                ...
                sens_bootstraplist = []
                prec_bootstraplist = []
            elif "wireless" in config["data_name"]:
                stats = eval_heartbeat_detection(imputation=imputation, target_seq=Y_dict_test["target_seq"], 
                                                 input=X_test, path=path, return_stats=True)
                sens = np.nanmean(stats[:,0])
                prec = np.nanmean(stats[:,1])
